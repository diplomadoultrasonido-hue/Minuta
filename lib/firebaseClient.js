...
/**
 * Registra el service worker y espera la sincronización COMPLETA del
 * navegador (no solo que registration.active exista). Revisar únicamente
 * `.active` no basta: Chrome puede marcarlo activo en el objeto de la
 * página antes de que el proceso del navegador termine de habilitarlo para
 * el Push API, lo que dispara "Subscription failed - no active Service
 * Worker" incluso con el worker ya activo. `navigator.serviceWorker.ready`
 * es la única forma garantizada por el spec de evitar esa carrera.
 */
async function registerAndWaitActive() {
...
export async function initPushNotifications() {
  try {
    ...
    const registration = await registerAndWaitActive();
