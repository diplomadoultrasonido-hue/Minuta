...
export async function updateCompromiso(username, id, updates) {
  ...
  if (Array.isArray(updates.assignedTo) && all[idx].assignedBy === username) {
    // Solo quien creó el compromiso puede reasignarlo a otras personas.
    const asignables = await getAssignableUsers(username);
    const validUsernames = new Set(asignables.map((a) => a.username));
    const assignedTo = updates.assignedTo.filter((u) => validUsernames.has(u));
 ...
 * /setup.
 */
export async function migrateLegacyDataToUser(username) {
  const all = await readAll();
  let migrados = 0;
  all.forEach((c) => {
    if (!c.assignedBy) {
