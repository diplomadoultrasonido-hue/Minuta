...
/**
 * Importación única de datos iniciales (por ejemplo, desde el Excel original).
 * Por seguridad, se niega a hacer nada si ya hay compromisos guardados —
 * así no hay riesgo de duplicar aunque se llame varias veces por error.
 */
export async function importIfEmpty(compromisosIniciales) {
  ...
  const client = await getClient();
  await client.set(KEY_COMPROMISOS, JSON.stringify(compromisosIniciales));
