...
import { getAssignableUsers, getOverseenUsernames, getUser, getCustomTipos } from './users';
...
async function canModify(c, username) {
  // Solo puede modificar (estado, prioridad, tipo, fecha, reasignar, eliminar):
  // quien está en "solicitado por", o un administrador que supervise a quien
  // lo solicitó, a quien lo creó, o a alguno de los asignados. Ser solo quien
  // lo creó (assignedBy) o solo estar en "asignado a" YA NO da permiso por sí
  // solo.
  if ((c.solicitadoPorUsernames || []).includes(username)) return true;

  const set = await getOverseenUsernames(username); // vacío si no administra ningún equipo
  if (set.has(c.assignedBy)) return true;
  if ((c.solicitadoPorUsernames || []).some((u) => set.has(u))) return true;
...
export async function getData(username) {
  ...
  const tiposSet = new Set(await getTiposFor(username));
  visible.forEach((c) => {
    if (c.tipo) tiposSet.add(c.tipo);
  });
...
const KEY_RECORDATORIOS_ENVIADOS = 'recordatorios_enviados';

/**
 * Compromisos cuya promesaCierre es EXACTAMENTE mañana y que no están
 * cerrados — usado por el cron diario de notificaciones.
 */
export async function getComprosisosPorVencerManana() {
  const all = await readAll();
  const manana = new Date();
  manana.setHours(0, 0, 0, 0);
  manana.setDate(manana.getDate() + 1);
  const mananaStr = fmt(manana);
  return all.filter((c) => c.status !== 'Cerrado' && c.promesaCierre === mananaStr);
}

/**
 * Evita mandar el mismo recordatorio dos veces (p.ej. si el cron se
 * ejecutara más de una vez el mismo día).
 */
export async function yaSeEnvioRecordatorio(compromisoId, fecha) {
  const client = await getClient();
  return client.sIsMember(KEY_RECORDATORIOS_ENVIADOS, `${compromisoId}-${fecha}`);
}

export async function marcarRecordatorioEnviado(compromisoId, fecha) {
  const client = await getClient();
  await client.sAdd(KEY_RECORDATORIOS_ENVIADOS, `${compromisoId}-${fecha}`);
}

export async function deleteCompromiso(username, id) {
  const all = await readAll();
  const idx = all.findIndex((c) => String(c.id) === String(id));
  if (idx === -1) throw new Error('Compromiso no encontrado');
  const puede = await canModify(all[idx], username);
  if (!puede) {
    throw new Error('Solo "solicitado por" o el administrador del equipo pueden eliminar este compromiso.');
  }
  all.splice(idx, 1);
  await writeAll(all);
  return getData(username);
