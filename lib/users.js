...
async function addUserToIndex(username) {
  const client = await getClient();
  const raw = await client.get(KEY_USERS_INDEX);
  const usernames = raw ? JSON.parse(raw) : [];
  if (!usernames.includes(username)) {
    usernames.push(username);
    await client.set(KEY_USERS_INDEX, JSON.stringify(usernames));
  }
}

export async function listAllUsers() {
  // Derivamos la lista de usuarios recorriendo todos los grupos (más confiable
  // que un índice aparte, que podía quedar desactualizado con cuentas viejas).
  const groups = await listGroups();
  const usernamesSet = new Set();
  for (const g of groups) {
    const members = await getGroupMembers(g.groupId);
    members.forEach((m) => usernamesSet.add(m.username));
  }
  const raw = await (await getClient()).get(KEY_USERS_INDEX);
  (raw ? JSON.parse(raw) : []).forEach((u) => usernamesSet.add(u));

  const users = await Promise.all(
    Array.from(usernamesSet).map(async (u) => {
      const user = await getUser(u);
      if (!user) return null;
      const userGroups = await getUserGroups(u);
      const groupsWithNames = await Promise.all(
        userGroups.map(async (g) => {
          const meta = await getGroupMeta(g.groupId);
          return { groupId: g.groupId, role: g.role, groupName: meta ? meta.name : g.groupId };
        })
      );
      return { username: user.username, name: user.name, canCreateGroups: !!user.canCreateGroups, groups: groupsWithNames };
    })
  );
...
/**
 * A diferencia de getAssignableUsers (que SIEMPRE incluye a la propia persona,
 * para poder auto-asignarse cosas), esta función es solo para calcular
 * permisos de supervisión: regresa el conjunto de usuarios que están en algún
 * equipo que este usuario administra. Si no administra ningún equipo, regresa
 * un conjunto vacío — nunca incluye a la propia persona "por defecto".
 */
export async function getOverseenUsernames(username) {
  const adminGroups = await getGroupsWhereAdmin(username);
  const set = new Set();
  for (const g of adminGroups) {
    const members = await getGroupMembers(g.groupId);
    members.forEach((m) => set.add(m.username));
  }
  return set;
}

/**
 * Quita a una persona de un grupo (deja de poder asignarle cosas nuevas de
 * ahí). Los compromisos que ya le habían asignado desde ese grupo NO se ven
 * afectados — la visibilidad vive en cada compromiso, no en la membresía.
 */
export async function removeMemberFromGroup({ groupId, username }) {
  const uname = String(username).toLowerCase().trim();
  const client = await getClient();

  const rawMembers = await client.get(keyGroupMembers(groupId));
  const members = rawMembers ? JSON.parse(rawMembers) : [];
  const newMembers = members.filter((m) => m.username !== uname);
  await client.set(keyGroupMembers(groupId), JSON.stringify(newMembers));

  const groups = await getUserGroups(uname);
  const newGroups = groups.filter((g) => g.groupId !== groupId);
  await setUserGroups(uname, newGroups);
}

export async function renameUser(username, newName) {
  const user = await getUser(username);
  if (!user) throw new Error('Usuario no encontrado');
...
/**
 * Un administrador agrega su propia opción de "Tipo" (ej. "Seguimiento
 * clínico"), disponible para él y para su equipo al crear/editar compromisos.
 */
export async function addCustomTipo(username, tipo) {
  const nombre = String(tipo || '').trim();
  if (!nombre) throw new Error('Escribe un nombre para el nuevo tipo');
  const user = await getUser(username);
  if (!user) throw new Error('Usuario no encontrado');
  const actuales = Array.isArray(user.customTipos) ? user.customTipos : [];
  if (!actuales.includes(nombre)) actuales.push(nombre);
  user.customTipos = actuales;
  await saveUser(user);
  return actuales;
}

// ---- Tokens de notificaciones push (Firebase Cloud Messaging) ----
// Un usuario puede tener varios tokens (uno por navegador/dispositivo donde
// dio permiso de notificaciones).
function keyUserFcmTokens(username) {
  return `user:${username.toLowerCase()}:fcmtokens`;
}

export async function addFcmToken(username, token) {
  if (!username || !token) return;
  const client = await getClient();
  const raw = await client.get(keyUserFcmTokens(username));
  const tokens = raw ? JSON.parse(raw) : [];
  if (!tokens.includes(token)) {
    tokens.push(token);
    await client.set(keyUserFcmTokens(username), JSON.stringify(tokens));
  }
}

export async function removeFcmToken(username, token) {
  if (!username || !token) return;
  const client = await getClient();
  const raw = await client.get(keyUserFcmTokens(username));
  const tokens = raw ? JSON.parse(raw) : [];
  ...
  if (filtered.length !== tokens.length) {
    await client.set(keyUserFcmTokens(username), JSON.stringify(filtered));
  }
}

export async function getFcmTokens(username) {
  const client = await getClient();
  const raw = await client.get(keyUserFcmTokens(username));
  return raw ? JSON.parse(raw) : [];
}

export async function getCustomTipos(username) {
  const user = await getUser(username);
  return user && Array.isArray(user.customTipos) ? user.customTipos : [];
}
